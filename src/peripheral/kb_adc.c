...
/* Changing base name. Used with kb_msg(). See @kb_base.h */
#ifdef KB_MSG_BASE
    #undef KB_MSG_BASE
...
static ADC_TypeDef *_get_device(kb_adc_device_t dev);
static uint32_t _get_channel(kb_adc_ch_t ch);
/* Peripheral clock function */
static void _enable_adc_clk(kb_adc_device_t dev);
...
/* Operation functions */
uint32_t kb_adc_measure(kb_adc_t *obj)
{   
